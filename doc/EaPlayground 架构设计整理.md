...
- 持有或配置 strategy registry。
- 创建或配置 strategy host、lifecycle 与 dispatcher。
- 将 `LauncherBundle`、`RuntimeState`、registry、strategy host、dispatcher 连接起来。
- 完成启动装配并返回启动结果。
...
- launcher 不负责分发，只负责装配 registry、host、dispatcher 和 lifecycle。
...
- `CLauncher::Bundle()` 返回 launcher-owned bundle 的引用。
- 调用方可以在 `Launch()` 前写入启动配置。
- 调用方不得在 launcher 生命周期之外保存 `Bundle()` 返回的引用。
- launcher 负责创建、校验、补齐 bundle。
...
Launcher
  ...
  owns Lifecycle
  owns StrategyHost
  wires references between runtime modules
...
Launcher releases Lifecycle / Dispatcher / Registry / Bundle / RuntimeState
```
...
- `EaPlayground initializing`：`.mq5` 到 launcher 的启动链路已触发。
...
- launcher 不负责事件分发，也不创建策略实例，只负责装配 registry、host、lifecycle、dispatcher、bundle 和 state。
...
- ownership 明确：launcher 拥有运行时组件，strategy host 拥有策略实例，strategy 拥有 chain，chain 拥有 handlers，context 只持有非 owning pointer。
